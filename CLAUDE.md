...
- Format: `pnpm format` — `prettier --write .`; `pnpm format:check` is the first step of the
  full check and of CI. `.prettierrc`: `printWidth` 100, `embeddedLanguageFormatting` off
  (`proseWrap` preserve and LF are the defaults). `.prettierignore`: `dist/`, `pnpm-lock.yaml`,
  the generated root `.claude/` and `docs/guides/agent-workflow.md`,
  `template/.claude/settings.json` (the CLI serializes it), and every process record
  (`docs/history/`, `docs/reviews/`, `docs/plans/`, `docs/reports/`)
- Unit: `pnpm test` — `node --test` over `test/**/*.test.ts`; black-box tests that spawn a hook
  with `CLAUDE_PROJECT_DIR` pointing at a fixture built under the OS temp dir
...
- Typecheck: `pnpm typecheck` — `tsc --noEmit` over the same files plus `test/`, `strict` plus
...
- Formatting is Prettier's, not yours: `pnpm format` runs before the final commit of a
  contribution (the orchestrator's close-out; implementers format the files they own first), and
  `pnpm format:check` fails CI on anything it would change. Format `template/`, never the root
  copy — `/.claude/` and the root guide are `.prettierignore`d and regenerated by `update`;
  `template/.claude/settings.json` stays in `JSON.stringify` shape because `mergeSettings`
  re-serializes it on every `update`; the two literal tables (`CASES` in `rule-zero-selftest.ts`,
  `COMMENT` in `docs-only.ts`) carry `// prettier-ignore`. After formatting `src/cli.ts`, rebuild
