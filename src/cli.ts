...
const FAILS_OPEN =
  "A hook whose script path is wrong fails OPEN — Claude Code reports it as a non-blocking\n" +
...
type FileClass = "managed" | "owned" | "merged";

/** One payload file: where it lives in the kit, where it lands, and who owns it. */
type PayloadFile = {
  /** template-relative source path, POSIX separators */
  src: string;
  /** target-relative destination path, POSIX separators (`gitignore` → `.gitignore`) */
  rel: string;
  cls: FileClass;
};

type Lock = {
  kitVersion: { version: string; sha: string | null };
  hashAlgo: string;
...
function abs(targetDir: string, rel: string): string {
...
function walk(dir: string, base: string): string[] {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (entry.isFile()) out.push(toPosix(path.relative(base, p)));
  }
  return out;
}
 ...
 */
function targetRel(src: string): string {
  const cut = src.lastIndexOf("/") + 1;
  return src.slice(cut) === "gitignore" ? src.slice(0, cut) + ".gitignore" : src;
}

function classify(src: string): FileClass {
  if (src === MERGED) return "merged";
  return OWNED.indexOf(src) >= 0 ? "owned" : "managed";
}

/** The manifest, derived from `template/` on every run. Never a hand-kept list. */
function payload(templateDir: string): PayloadFile[] {
  const files = walk(templateDir, templateDir).map(function (src): PayloadFile {
...
function hooksManifest(files: readonly PayloadFile[]): string[] {
...
function stemSet(manifest: readonly string[]): Set<string> {
 ...
 */
function installedSha(root: string, name: string): string | null {
  const candidates = [
  ...
  for (const candidate of candidates) {
    ...
    for (const key of Object.keys(packages)) {
      ...
      if (key !== tail && !key.endsWith("/" + tail)) continue;
...
function loadKit(): Kit {
  ...
  if (!fs.existsSync(manifestPath)) die(`no package.json at ${root} — the package is incomplete`);
  const pkg: unknown = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
  ...
  if (!fs.existsSync(templateDir)) die(`no template/ at ${root} — the package is incomplete`);
  return {
    root: root,
...
function hookScripts(entry: unknown): string[] {
...
function mergeSettings(templateText: string, currentText: string | null, stems: Set<string>): string {
  ...
  const merged: Record<string, unknown> = isRecord(parsedCurrent) ? parsedCurrent : {};
  ...
  if (isRecord(templateWorktree) && "baseRef" in templateWorktree) {
    ...
    const worktree: Record<string, unknown> = isRecord(existing) ? existing : {};
  ...
  if (isRecord(templateHooks)) {
    ...
    const hooks: Record<string, unknown> = isRecord(existingHooks) ? existingHooks : {};
    for (const event of Object.keys(templateHooks)) {
      const templateGroups = templateHooks[event];
      if (!Array.isArray(templateGroups)) continue;
      ...
      for (const templateGroup of templateGroups) {
        ...
        const group = groups[at] as Record<string, unknown>;
        const existing = Array.isArray(group["hooks"]) ? (group["hooks"] as unknown[]) : [];
        const rebuilt: unknown[] = [];
        let placed = false;
        for (const entry of existing) {
          if (isKitHookEntry(entry, stems)) {
            if (!placed) {
              rebuilt.push(...(clone(kitEntries) as unknown[]));
              placed = true;
            }
        ...
        if (!placed) rebuilt.push(...(clone(kitEntries) as unknown[]));
        group["hooks"] = rebuilt;
      }
      hooks[event] = groups;
    }
...
function cmdInit(targetDir: string): number {
  const kit = loadKit();
  ...
  if (beside > 0) say(`  review the .new files, then delete them — nothing was overwritten`);
  return 0;
}

/* ----------------------------------------------------------------------- update */

function cmdUpdate(targetDir: string): number {
  const kit = loadKit();
  const files = payload(kit.templateDir);
  const manifest = hooksManifest(files);
  const stems = stemSet(manifest);
  const previous = loadLock(targetDir);
  const recorded: Record<string, string> = previous === null ? {} : { ...previous.files };
  let refreshed = 0;
  let current = 0;
  let edited = 0;
  let owned = 0;
  ...
  say(
    `update: ${refreshed} refreshed, ${current} already current, ${edited} left beside as .new, ` +
      `${removed} removed, ${owned} owned files untouched`,
  );
  if (edited > 0) say(`  merge each .new by hand, then delete it — nothing was overwritten`);
  return 0;
}

/* ----------------------------------------------------------------------- doctor */

type Doctor = { passed: number; failed: number };

function pass(state: Doctor, line: string): void {
  state.passed++;
  say(`  ok     ${line}`);
}

function fail(state: Doctor, line: string, detail?: string): void {
  state.failed++;
  say(`  FAIL   ${line}`);
  if (detail !== undefined) say(`         ${detail}`);
}

function cmdDoctor(targetDir: string): number {
  const kit = loadKit();
  const files = payload(kit.templateDir);
  const manifest = hooksManifest(files);
  const stems = stemSet(manifest);
  const state: Doctor = { passed: 0, failed: 0 };
  ...
  if (major >= MIN_NODE_MAJOR) pass(state, `node v${process.versions.node} (>= ${MIN_NODE_MAJOR})`);
  else fail(state, `node v${process.versions.node} — the kit needs Node >= ${MIN_NODE_MAJOR}`);
  ...
  if (lock === null) {
    ...
    pass(
      state,
      `${LOCK_REL} — kit ${versionLabel(lock.kitVersion)}, ${Object.keys(lock.files).length} files, ${lock.hashAlgo}`,
    );
  }
  ...
  if (!fs.existsSync(shim)) {
    ...
    if (type === "module") pass(state, `.claude/hooks/package.json — {"type":"module"}`);
    else fail(state, `.claude/hooks/package.json does not set "type":"module"`, FAILS_OPEN);
  }
  ...
  if (!fs.existsSync(selftest)) {
    ...
    if (run.status !== 0 || counts === null) {
      ...
      fail(
        state,
        `self-test ran ${counts[2]} cases, this kit expects ${EXPECTED_SELFTEST_CASES}`,
        "the hooks and the CLI are from different kit versions — run `cl-workflow update`",
      );
    } else {
...
function main(argv: readonly string[]): number {
  ...
  if (!fs.existsSync(targetDir)) die(`${targetDir} does not exist`);
  if (command === "update") return cmdUpdate(targetDir);
  if (command === "doctor") return cmdDoctor(targetDir);
